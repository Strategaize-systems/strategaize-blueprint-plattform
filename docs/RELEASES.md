...
### REL-010 — V3.4 Feedback + Compliance (FINAL Blueprint Release — Freeze)
- Date: 2026-04-14
- Scope: FEAT-036 Feedback-Schleife + FEAT-038 Compliance-Datentrennung. 3 Slices (SLC-062 Backend, SLC-063 Frontend, SLC-064 Export). Neue Tabelle run_feedback (Migration 020) mit 4 festen Fragen (Coverage, Clarity, Improvements, Overall mit 1-5 Rating). Depersonalisierung by Design (DEC-043: kein created_by). Feedback-Tab im Workspace dynamisch aktivierbar (tenant_admin + submitted/locked + management). UPSERT-Logik (DEC-045). Export-Haertung: Mirror-Export enthaelt KEINEN freeform/ Ordner (DEC-044), Management-Export enthaelt freeform/ Conversations + feedback.json, Manifest mit has_feedback + includes_freeform Flags. Neue Datei docs/COMPLIANCE.md dokumentiert Datentrennung, Zugriffsmatrix und Export-Regeln. IMP-032, IMP-033, IMP-034 als Skill-Improvements. Tab-Verhalten bei locked Runs geaendert (Offen disabled, Feedback enabled). Rule sql-migration-hetzner.md im Dev-System angelegt.
- Summary: Letzter Blueprint-Release vor strategischem Freeze. V3.4 schliesst den V3-Zyklus mit Feedback-Mechanik und expliziter Compliance-Dokumentation ab. Endkunde sieht Feedback-Tab nach Fragebogen-Abschluss, kann 4 Fragen beantworten (3x Freitext + 1x Rating+Text), Daten landen depersonalisiert in run_feedback und werden mit den Export-ZIPs mitgegeben. Management-Export enthaelt zusaetzlich die Freeform-Rohgespraeche, Mirror-Export explizit NICHT (Vertraulichkeitsgarantie fuer Mitarbeiter). Mit diesem Release wird Blueprint als eigenstaendiges Produkt eingefroren — Weiterentwicklung erfolgt in der neuen Onboarding-Plattform (separates Repo).
- Risks: Keine automatisierten Tests (ISSUE-002, seit MVP-1 akzeptiert). Freeform-API-Routen ohne expliziten survey_type-Check (IMP-033, dokumentiert in COMPLIANCE.md als Defense-in-Depth-Verbesserungspotential). BL-035 Lektorat, BL-045 DSGVO-Kundenuebergabe, BL-059 Policy-Text Recht bewusst offen beim Einfrieren — Blueprint wird nicht produktiv an Kunden uebergeben. Strategische Entscheidung: V4 Blueprint entfaellt, V4 Interne Meetings wandert in Onboarding-Plattform.
- Rollback Notes: Coolify Container-Rollback auf V3.3 Build (Commit 4b95f60 oder frueher). DB: DROP TABLE run_feedback CASCADE;
